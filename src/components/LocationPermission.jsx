 ...
 * Displays different UI states based on geolocation status
 */
export const LocationPermission = ({ status, error, location, onRetry, onContinue }) => {
  // Loading state
  if (status === 'loading') {
    return (
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full">
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            <p className="text-xs text-gray-600">
              <strong>Tip:</strong> To enable location access, check your browser settings 
              and look for site permissions.
            </p>
          </div>
        </div>
      </div>
  ...
  // Error state (other than permission denied)
  if (status === 'error') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-50 to-emerald-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
  ...
  // Success state - return null as main app will render
  return null;
};